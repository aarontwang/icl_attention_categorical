...
def load_weights(save_path, kernel, seed, gd=False):
    with open("{}/{}/params_{}_seed_{}.pickle".format(save_path, kernel, "gd" if gd else "trained", seed), "rb") as f:
...
def predict_sample_space(rng, save_path, seed, params_tf, params_gd):
    ...
    c_idxs = scan_data[0]
    x = scan_data[1]
    x_prob = scan_data[5].squeeze()[0]
    x_labels = scan_data[2].squeeze()

    scan_data = scan_data[3:]

    full_preds_gd = predict.apply(params_gd, new_rng, scan_data, gd=True)
    full_preds_tf = predict.apply(params_tf, new_rng, scan_data, gd=False)
    ...
    plot_scan("{}".format(save_path), seed, x_query, full_preds_tf, full_preds_gd, x, x_labels, c_idxs)
...
def gen_scan_data(rng, x_query):
    ...
    zero = jnp.expand_dims(jnp.concatenate([x_query, jnp.zeros((x_query.shape[0], cats)) - 1 / cats,
                                             jnp.zeros((x_query.shape[0], e_size))], axis=-1), 1)

    seq = jnp.concatenate([seq, zero], axis=1)

    return jnp.squeeze(c_idx), jnp.squeeze(x), jnp.squeeze(y_data), jnp.squeeze(seq), jnp.squeeze(target), \
        jnp.squeeze(jnp.tile(probs, (x_query.shape[0], 1, 1))), \
...
def plot_scan(save_path, seed, x_query, preds_tf, preds_gd, x, x_label, c_idxs):
    ...
    fig6.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(0, max_q5), cmap=cm1),
