...
def predict_sample_space(rng, save_path, seed, params_tf, params_gd):
    ...
    full_preds_gd = predict_scan.apply(params_gd, new_rng, scan_data, gd=True)
    full_preds_tf = predict_scan.apply(params_tf, new_rng, scan_data, gd=False)
...
def gen_scan_data(rng, x_query):
    ...
    return jnp.squeeze(c_idx), jnp.squeeze(max_quad_idx), jnp.squeeze(x), jnp.squeeze(y_data), jnp.squeeze(seq), \
        jnp.squeeze(target), jnp.squeeze(jnp.tile(probs, (x_query.shape[0], 1, 1))), \
