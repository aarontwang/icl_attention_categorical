...
class TransformerGDFeedforward(nn.Module):
    ...
    def __init__(
        ...
        self.alpha = nn.Parameter(torch.randn((self.num_layers, self.num_heads, self.emb_size), generator=gen).to(self.device))
        ...
        self.W_Q = torch.zeros((self.num_heads+1, self.input_size + 3 * self.emb_size, self.input_size + 3 * self.emb_size)).to(
            self.device)
        self.W_K = torch.zeros((self.num_heads+1, self.input_size + 3 * self.emb_size, self.input_size + 3 * self.emb_size)).to(
            self.device)
        ...
        self.W_V = torch.zeros((self.num_heads+1, self.input_size + 3 * self.emb_size, self.input_size + 3 * self.emb_size)).to(
            self.device)
        ...
        self.P = torch.zeros((self.num_heads+1, self.input_size + 3 * self.emb_size, self.input_size + 3 * self.emb_size)).to(
            self.device)
        ...
        if self.shared_ff:
            # input layer
            layer_list = [nn.Linear(2 * self.emb_size, self.ff_hidden_size, bias=True)]
            ...
            ff_weight_list.append(nn.ModuleList(layer_list))
        else:
    ...
    def self_attn(self, key, query, value, layer):
        ...
        if self.kernel == 'linear':
            scores_1 = self.kernel_param.view(1, -1, 1, 1) * torch.einsum('bhim,bhin -> bhmn', Q[:, 0:self.num_heads], K[:, 0:self.num_heads])
    ...
    def feedforward(self, h, layer):
        ...
        if self.shared_ff:
            for ff_layer in range(self.num_ff_layers):
                h_out = self.ff_weights[0][ff_layer](h_out)
                h_out = F.gelu(h_out)

            h_out = self.ff_weights[0][-1](h_out)
        else:
            for ff_layer in range(self.num_ff_layers):
                h_out = self.ff_weights[layer][ff_layer](h_out)
                h_out = F.gelu(h_out)

            h_out = self.ff_weights[layer][-1](h_out)

        # from shape (batch_size, n + 1, hidden_size) to (batch_size, emb_size, n + 1)
        return torch.permute(h_out, (0, 2, 1))
    ...
    def train_step(self, features, labels, return_probs=False):
        # get query labels
        ...
        if return_probs:
            return query_loss, query_accuracy, query_preds
        else:
...
class TransformerFeedforward(nn.Module):
    def __init__(
        ...
        self.W_Q = nn.Parameter(
            torch.randn((self.num_layers, self.num_heads + 1, self.input_size + 3 * self.emb_size, self.input_size + 3 * self.emb_size),
                        generator=gen).to(
