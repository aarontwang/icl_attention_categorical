...
def plot_comparison(save_path, linear, exp, rbf, lap, softmax, max_prob=None,
    ...
    # Training Epochs
    x_range = np.arange(0, num_iter_os, int(num_iter_os / len(softmax)))
...
def compare_kernels(save_path, linear=False, exp=False, rbf=False, laplacian=False,
    ...
    if linear:
        linear_loss_trans_list, linear_losses_gd_list, linear_prob_loss_trans_list, linear_prob_loss_gd_list, \
            linear_acc_trans_list, linear_acc_gd_list, linear_top_3_freq_trans_list, linear_top_3_freq_gd_list, \
            linear_max_prob = extract_data(save_path, 'linear', False, 5000, train)

    if rbf:
        rbf_loss_trans_list, rbf_losses_gd_list, rbf_prob_loss_trans_list, rbf_prob_loss_gd_list, \
            rbf_acc_trans_list, rbf_acc_gd_list, rbf_top_3_freq_trans_list, rbf_top_3_freq_gd_list, \
            rbf_max_prob = extract_data(save_path, 'rbf', False, 5000, train)

    if softmax:
        softmax_loss_trans_list, softmax_losses_gd_list, softmax_prob_loss_trans_list, softmax_prob_loss_gd_list, \
            softmax_acc_trans_list, softmax_acc_gd_list, softmax_top_3_freq_trans_list, softmax_top_3_freq_gd_list, \
...
def compare_layers(save_path, data_path_1, data_path_2, data_path_3, data_path_4, data_path_5, data_path_6, early_stopping=False):
    data_names = [('loss_trans_list', 'gd_train_loss_list'), ('acc_trans_list', 'gd_train_acc_list'),
                  ('top_3_freq_trans_list', 'gd_train_top_3_freq_list'),
                  ('prob_dist_trans_list', 'gd_train_prob_dist_list')]
    ...
    y_lim_l = [0.85, 0, 0, 0]
    y_lim_u = [1.5, 0.8, 1, 0.3]

    y_lim_l_tf = [0.9, 0, 0, 0]
    y_lim_u_tf = [3.0, 1, 1, 0.3]
    ...
    for j, kernel in enumerate(kernels):
        for i, (data_tf, data_gd) in enumerate(data_names):
            print(data_tf)
            plot_comparison_multiple_layers([data_path_1, data_path_2, data_path_3, data_path_4, data_path_5, data_path_6],
                                            save_path,
                                            data_tf, data_gd, layers, kernel,
                                            labels[j],
                                            num_iter_os_1=5000, num_iter_os_2=5000,
                                            plot_title=None, title=save_titles[i], x_label="Training Epochs",
                                            y_label=plot_titles[i], yscale_log=False, y_lim_l=y_lim_l[i],
                                            y_lim_u=y_lim_u[i], y_lim_l_tf=y_lim_l_tf[i], y_lim_u_tf=y_lim_u_tf[i],
...
def plot_comparison_multiple_layers(data_paths, save_path, data_tf, data_gd, layers, kernel, kernel_label,
    ...
    for layer in layers:
        data_f = open(data_paths[layer-1] + '/{}/results.json'.format(kernel))
        ...
        if early_stopping:
            ...
            for i, idx in enumerate(best_idx_trans_list):
                idx = idx // 100
                gd_idx = best_idx_gd_list[i] // 100

                trans_list[i][idx + 1:] = [trans_list[i][idx]] * (steps - idx - 1)
                gd_list[i][gd_idx + 1:] = [gd_list[i][gd_idx]] * (steps - gd_idx - 1)
        ...
        # Epochs
        x_range_1 = np.arange(0, num_iter_os_1, int(num_iter_os_1 / len(trans_list_mean)))
        x_range_2 = np.arange(0, num_iter_os_2, int(num_iter_os_2 / len(gd_list_mean)))
        ...
        ax2.plot(x_range_2, gd_list_mean, color=colors(color_vals[layer-1] + color_add), label="{} Layer".format(layer),
                 linewidth='3')
    ...
    legend1 = ax1.legend(loc=loc_first, framealpha=0.99, facecolor='white', ncol=2)
    legend1.set_zorder(100)
    legend2 = ax2.legend(loc=loc_first, framealpha=0.99, facecolor='white', ncol=2)
...
def context_comparison(save_path, linear=False, exp=False, rbf=False, laplacian=False,
    ...
    data_names = [('loss_trans_list', 'loss_gd_list'), ('acc_trans_list', 'acc_gd_list'),
                  ('top_3_freq_trans_list', 'top_3_freq_gd_list'),
                  ('prob_dist_trans_list', 'prob_dist_gd_list')]
    plot_titles = ["Negative Log-Likelihood Loss", "Accuracy", "Top 3 Frequency", "MSE on Category Probabilities"]
...
def extract_data(data_path, model_kernel, early_stopping=False, steps=5000, train=False):
    ...
    losses_gd_list = list(filter(None, data['gd_train_loss_list'] if train else data['losses_gd_list']))
    ...
    acc_gd_list = list(filter(None, data['gd_train_acc_list'] if train else data['acc_gd_list']))
    ...
    if early_stopping:
        best_idx_trans_list = data['best_idx_trans_list']
        best_idx_gd_list = data['gd_val_best_steps']
        for i, idx in enumerate(best_idx_trans_list):
            idx = idx // 100
            gd_idx = best_idx_gd_list[i] // 100
    ...
    return loss_trans_list, losses_gd_list, prob_loss_trans_list, prob_loss_gd_list, acc_trans_list, acc_gd_list, \
        top_3_freq_trans_list, top_3_freq_gd_list, max_prob
    ...
    for j, model_kernel in enumerate(kernels):
        ...
        if not imagenet:
            ...
            display_training(save_path, gd_train_prob_dist, trained_tf=prob_loss_trans_list,
                             num_iter_os=len(prob_loss_trans_list[0]) * 100,
                             plot_title=None,
        ...
        display_training(save_path, gd_train_top_3_freq, trained_tf=top_3_freq_trans_list,
                         num_iter_os=len(top_3_freq_gd_list[0]) * 100,
                         plot_title=None,
...
def plot_linear_training(save_path, data_path, early_stopping=True, linear=False, exp=False, rbf=False,
    ...
    for j, model_kernel in enumerate(kernels):
        ...
        loss_trans_list = np.expand_dims(np.array(data['loss_trans_list']).mean(axis=0), axis=0)
        losses_gd_list = data['losses_gd_list']
        acc_trans_list = np.expand_dims(np.array(data['acc_trans_list']).mean(axis=0), axis=0)
        acc_gd_list = data['acc_gd_list']
        ...
        gd_train_loss = data['gd_train_loss_list']
        if not imagenet:
            gd_train_prob_dist = data['gd_train_prob_dist_list']
        gd_train_acc = data['gd_train_acc_list']
...
def display_training(save_path, gd, trained_tf=None, gd_val=None, max_prob=None,
    ...
    # Epochs
    x_range = np.arange(0, num_iter_os, int(num_iter_os / len(trained_tf)))
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'plot_linear_training':
        ...
        compare_layers(sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], bool(int(sys.argv[9])))
